...
object CalPeakHourFactorAlgorithm {
  ...
  /***
    ...
    * @return
    */
  def CalPeakHour15Factor(spark: SparkSession, rdd: RDD[sizeflow15min]): RDD[String] = {
    ...
    val data = rdd.toDF().rdd.map(s => {
      ...
      val getPeak = GetPeakHour15min(timeAndFlow)
  ...
  def CalPeakHourFactor(spark: SparkSession, rdd: RDD[sizeflow15min]): RDD[String] = {
    ...
    val data = rdd.toDF().rdd.map(s => {
      ...
      station_name+","+getPeak._1+","+getPeak._2+","+getPeak._3+","+getPeak._4+","+getPeak._5+","+getPeak._4*1.0/getPeak._5
    })
    data
  }
  ...
  /***
    ...
    */
  def GetPeakHour15min(array: Array[(String,Long)]): (String,Long,String,Long) = {
    try {
      var index = 0
  ...
  def GetPeakHour(array: Array[(String,Long)]): (String,Long,String,Long,Long) = {
    try {
      var index = 0
      var PeakFlow = array(0)._2
      var PeakFlowTime = index
      var PeakHourFlow = array(0)._2 + array(1)._2 + array(2)._2 + array(3)._2
      var DayFlow = array(0)._2
      for (j <- 1 until array.length - 4) {
        val tmp = array(j)._2 + array(j + 1)._2 + array(j + 2)._2 + array(j + 3)._2
        if (PeakHourFlow < tmp) {
          //如果tmp大于Peak，则将tmp的值传给Peak，并将j值记录下来
      ...
      for(n <- 1 until array.length-1){
        DayFlow += array(n)._2
      }
      (array(index)._1, PeakFlow, array(PeakFlowTime)._1, PeakHourFlow, DayFlow) //高峰小时的开始索引值，最高峰值，最高峰值时间索引值，高峰小时客流值
    }catch {
      case e:ArrayIndexOutOfBoundsException =>
        e.printStackTrace()
        array.foreach(println)
        (array(0)._1, array(0)._2, array(0)._1, array(0)._2, array(0)._2)
    }
  }
}
