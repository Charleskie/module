...
/**
  * BusO站点匹配
  */
class BusOCal extends Serializable{
  /***
    *  把车牌号格式化
    */
...
object BusOCal{
  def apply(): BusOCal = new BusOCal()
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().master("local[*]").getOrCreate()
    val sc = spark.sparkContext
    val date1 = "20170628"
    val date2 = "2017-06-28"
    val date3 = "2017-06-29"
    val deal = sc.textFile("E:\\Portable\\sibat\\spark\\module\\testdata\\BusCal\\"+date1)
    val GPS = sc.textFile("E:\\Portable\\sibat\\spark\\module\\testdata\\BusCal\\ARRLEA_Q_20170628,E:\\Portable\\sibat\\spark\\module\\testdata\\BusCal\\ARRLEA_Q_20170629")
    //  val DealRDD =  BusOCal().CalBusStationGPS(GPS,"1,2,3,5,4,10,8").foreach(println)//.coalesce(1).saveAsTextFile("G:\\数据\\BusO\\buyaode\\GPS")
    /*val orginDealRDD = deal.count()
    val DealRDD = BusOCal().CalBusDeal(deal,"1,7,4").count()
    println("origin:"+orginDealRDD+","+"get:"+DealRDD)
    println(orginDealRDD-DealRDD)
    val GPSRDD =  BusOCal().CalBusStationGPS(GPS,"1,2,3,5,4,10,8").count()
    val orginGPSRDD = GPS.count()
    println("origin:"+orginGPSRDD+","+"get:"+GPSRDD)
    println(orginGPSRDD-GPSRDD)*/
    BusOCal().CalBusO(deal,"1,7,4",GPS,"1,2,3,5,4,10,8").coalesce(1).saveAsTextFile("E:\\Portable\\sibat\\spark\\module\\testdata\\BusCal\\output\\"+date1)
    //car_id,arrive_time,leave_time,line,direction,devide,station_id
    val DealRDD = BusOCal().CalBusDeal(deal,"1,7,4")
    val grpDeal = DealRDD.groupBy(_.car_id)
    val DealRDD_o = DealRDD.count()
    val grpDeal_count = grpDeal.count()

    val GPSRDD =  BusOCal().CalBusStationGPS(GPS,"1,2,3,5,4,10,8")
